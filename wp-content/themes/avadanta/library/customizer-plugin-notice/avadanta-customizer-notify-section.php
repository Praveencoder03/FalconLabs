...
class Avadanta_Customizer_Notify_Section extends WP_Customize_Section {
	
	public $type = 'avadanta-customizer-notify-section';
	...
	public function call_plugin_api( $slug ) {
		...
		$call_api = get_transient( 'avadanta_cust_notify_plugin_info_' . $slug );
		if ( false === $call_api ) {
			...
			set_transient( 'avadanta_cust_notify_plugin_info_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
		}
	...
	public function json() {
		...
		$avadanta_customizer_notify_show_recommended_actions = get_option( 'avadanta_customizer_notify_show' );
		foreach ( $avadanta_customizer_notify_recommended_actions as $key => $avadanta_lite_customizer_notify_recommended_action ) {
			if ( $avadanta_customizer_notify_show_recommended_actions[ $avadanta_lite_customizer_notify_recommended_action['id'] ] === false ) {
				continue;
			}
			if ( $avadanta_lite_customizer_notify_recommended_action['check'] ) {
				continue;
			}

			$avadanta_lite_customizer_notify_recommended_action['index'] = $key + 1;

			if ( isset( $avadanta_lite_customizer_notify_recommended_action['plugin_slug'] ) ) {
				$active = $this->check_active( $avadanta_customizer_notify_recommended_action['plugin_slug'] );
				$avadanta_lite_customizer_notify_recommended_action['url'] = $this->create_action_link( $active['needs'], $avadanta_lite_customizer_notify_recommended_action['plugin_slug'] );
				if ( $active['needs'] !== 'install' && $active['status'] ) {
					$avadanta_lite_customizer_notify_recommended_action['class'] = 'active';
				} else {
					$avadanta_lite_customizer_notify_recommended_action['class'] = '';
				}

				switch ( $active['needs'] ) {
					case 'install':
						$avadanta_lite_customizer_notify_recommended_action['button_class'] = 'install-now button';
						$avadanta_lite_customizer_notify_recommended_action['button_label'] = $install_button_label;
						break;
					case 'activate':
						$avadanta_lite_customizer_notify_recommended_action['button_class'] = 'activate-now button button-primary';
						$avadanta_lite_customizer_notify_recommended_action['button_label'] = $activate_button_label;
						break;
					case 'deactivate':
						$avadanta_lite_customizer_notify_recommended_action['button_class'] = 'deactivate-now button';
						$avadanta_lite_customizer_notify_recommended_action['button_label'] = $avadanta_deactivate_button_label;
						break;
				}
			}
			$formatted_array[] = $avadanta_lite_customizer_notify_recommended_action;
		}// End foreach().

		$customize_plugins = array();

		$avadanta_lite_customizer_notify_show_recommended_plugins = get_option( 'avadanta_customizer_notify_show_recommended_plugins' );

		foreach ( $avadanta_customizer_notify_recommended_plugins as $slug => $plugin_opt ) {

			if ( ! $plugin_opt['recommended'] ) {
				continue;
			}

			if ( isset( $avadanta_lite_customizer_notify_show_recommended_plugins[ $slug ] ) && $avadanta_lite_customizer_notify_show_recommended_plugins[ $slug ] ) {
				continue;
			}
			...
			switch ( $active['needs'] ) {
				...
				case 'deactivate':
					...
					$ti_customizer_notify_recommended_plugin['button_label'] = $avadanta_deactivate_button_label;
					break;
			}
		...
		$json['total_actions']       = count( $avadanta_customizer_notify_recommended_actions );
		...
		<# if( data.recommended_actions.length > 0 || data.recommended_plugins.length > 0 ){ #>
			<li id="accordion-section-{{ data.id }}" class="accordion-section control-section control-section-{{ data.type }} cannot-expand">
				...
				<h3 class="accordion-section-title">
					...
					<# if( data.recommended_actions.length > 0 ){ #>
						<span class="avadanta-customizer-plugin-notify-actions-count">
							<span class="current-index">{{ data.recommended_actions[0].index }}</span>
							{{ data.total_actions }}
						</span>
					<# } #>
				</h3>
				<div class="avadanta-theme-recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_actions.length > 0 ){ #>
						<# for (action in data.recommended_actions) { #>
							<div class="avadanta-recommeded-actions-container epsilon-required-actions" data-index="{{ data.recommended_actions[action].index }}">
								<# if( !data.recommended_actions[action].check ){ #>
									<div class="avadanta-epsilon-recommeded-actions">
										<p class="title">{{ data.recommended_actions[action].title }}</p>
										<span data-action="dismiss" class="dashicons dashicons-no avadanta-customizer-notify-dismiss-recommended-action" id="{{ data.recommended_actions[action].id }}"></span>
					...
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="avadanta-recommeded-actions-container epsilon-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="avadanta-epsilon-recommeded-actions">
										<p class="title">{{ data.recommended_plugins[action].title }}</p>
										<span data-action="dismiss" class="dashicons dashicons-no avadanta-customizer-notify-dismiss-button-recommended-plugin" id="{{ data.recommended_plugins[action].id }}"></span>
